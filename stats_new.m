...
%% Hu stats
MARD_Hu = 10^(mean(abs(estimate_log10 - data_log10)));
MRD_Hu = mean(abs((estimate - data)./data));
Bias_Hu = 10^(mean(estimate_log10 - data_log10));
URMD_Hu = mean(abs((2.*(estimate - data))./(estimate + data)));
URMSD_Hu = sqrt(mean(((2.*(estimate - data))./(estimate + data))));
MeanRat_Hu = mean(estimate./data);
MedRat_Hu = median(estimate./data);
...
restats.MARD_Hu = MARD_Hu;
restats.MRT_Hu = MRD_Hu;
restats.Bias_Hu = Bias_Hu;
restats.URMD_Hu = URMD_Hu;
restats.URMSD_Hu = URMSD_Hu;
restats.MeanRat_Hu = MeanRat_Hu;
restats.MedRat_Hu = MedRat_Hu;
